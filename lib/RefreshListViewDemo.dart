...
//      _words.addAll(generateWordPairs().take(20).map(f));
      mController.addListener(() {
        if (mController.position.pixels ==
            mController.position.minScrollExtent) {
          setState(() {
            showRefreshLoad = true;
          });
          _toRefresh();
        }
      });
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        ...
        body: Stack(
          children: <Widget>[
            Offstage(
              offstage: showRefreshLoad ? false : true,
